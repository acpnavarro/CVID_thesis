...
dds2 <-DESeq(dds)
resultsNames(dds2)
res <- results(dds2, contrast = c("Group", "CVID_Nonactivated_BrightMBCs", "HD_Nonactivated_BrightMBCs"))
...
write.xlsx(DEGs,"C:/Users/Anna/Documents/CVID_project/DEGs/IB_CVIDxHD/pvalue0.05.xlsx", asTable = TRUE, rowNames=TRUE)
#Filter
top25_down <- DEGs %>%
...
write.xlsx(top50,"C:/Users/Anna/Documents/CVID_project/DEGs/IB_CVIDxHD/top50.xlsx", asTable = TRUE, rowNames=TRUE)
...
s_mat2<-s_mat[, grep("^IB", names(s_mat))]
...
df_IB=df_clinical %>% 
  filter(str_detect(row.names(df_clinical), "^IB"))
df_ann2=cbind(df_ann,df_IB)
...
heat_plot <- pheatmap(s_mat2,scale="none", clustering_method="average", name="Z score",
                      ...
                      show_colnames = T,
                      fontsize_col = 6,
                      main = "Nonactivated Bright Memory B cells") 
##################################################################
###volcano plot###
df=data.frame(res)
write.xlsx(df,"C:/Users/Anna/Documents/CVID_project/DF/df.IB_CVIDxHD.xlsx", asTable = TRUE, rowNames=TRUE)
# Theme
...
ggplot(data = df, aes(x = log2FoldChange, y = -log10(pvalue))) +
  ...
  labs(title="Nonactivated Bright Memory B cells: CVID x HD")+
