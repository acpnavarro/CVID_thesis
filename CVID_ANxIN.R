...
dds2 <-DESeq(dds)
resultsNames(dds2)
res <- results(dds2, contrast = c("Group", "CVID_Activated_NaïveBCs", "CVID_Nonactivated_NaïveBCs"))
...
s_mat2<-s_mat[, grep("^AN.CVID|^IN.CVID", names(s_mat))]
...
df_ann=ann_sample %>% 
  filter(str_detect(Sample, "^AN.CVID|^IN.CVID"))
df_ann <- data.frame(df_ann, row.names = 1)
df_ann$Group<-gsub("*_NaïveBCs", "", df_ann$Group)
...
heat_plot <- pheatmap(s_mat2,scale="none", clustering_method="average", name="Z score",
                      ...
                      show_colnames = T,
                      fontsize_col = 6,
                      main = "CVID Naïve B cells: Activated x Nonactivated")
##################################################################
###volcano plot###
df=data.frame(res)
write.xlsx(df,"C:/Users/Anna/Documents/CVID_project/DF/df.CVID_ANXIN.xlsx", asTable = TRUE, rowNames=TRUE)
# Theme
...
ggplot(data = df, aes(x = log2FoldChange, y = -log10(pvalue))) +
  ...
  labs(title="CVID Naïve B cells: Activated x Nonactivated")+
