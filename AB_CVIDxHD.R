...
dds2 <-DESeq(dds)
resultsNames(dds2)
res <- results(dds2, contrast = c("Group", "CVID_Activated_BrightMBCs", "HD_Activated_BrightMBCs"))
...
write.xlsx(DEGs,"C:/Users/Anna/Documents/CVID_project/DEGs/AB_CVIDxHD/pvalue0.5.xlsx", asTable = TRUE, rowNames=TRUE)
#Filter
top25_down <- DEGs %>%
...
write.xlsx(top50,"C:/Users/Anna/Documents/CVID_project/DEGs/AB_CVIDxHD/top50.xlsx", asTable = TRUE, rowNames=TRUE)
#######
top5_down <- DEGs %>%
  filter(log2FoldChange < -1.5) %>%
  arrange(desc(abs(log2FoldChange)))  %>%
...
top5_up<- DEGs %>%
  ...
  arrange(desc(abs(log2FoldChange)))  %>%
  dplyr::slice(1:5)
#data frame with 10 most DEGs according to log2FC
top10 <- rbind (top5_up, top5_down)
write.xlsx(top10,"C:/Users/Anna/Documents/CVID_project/DEGs/AB_CVIDxHD/top10.xlsx", asTable = TRUE, rowNames=TRUE)
######
...
#Z score
s_mat=t(scale(t(mat)))
# select only the activated memory cell samples patients from the count table
s_mat=as.data.frame(s_mat)
s_mat2<-s_mat[, grep("^AB", names(s_mat))]
...
df_AB=df_clinical %>% 
  filter(str_detect(row.names(df_clinical), "^AB"))
df_ann=rownames_to_column(df_ann)
colnames(df_ann)[colnames(df_ann) == "rowname"] <- "Sample" 
df_ann2=cbind(df_ann,df_AB)
df_ann2=df_ann2[,-1]

##Colors for the groups
color_ann = list(df_ann2, Group=c("CVID"  = "#048907",
                                 "HD" = "#890486"),
          Only_Infections=c("no"=	"#048689", "yes"="#894a04", " "="grey"))
...
heat_plot <- pheatmap(s_mat2,scale="none", clustering_method="average", name="Z score",
                      ...
                      show_colnames = T,
                      fontsize_col = 6,
                      main = "Activated Bright Memory B cells") 
##################################################################
###volcano plot###
df=data.frame(res)
write.xlsx(df,"C:/Users/Anna/Documents/CVID_project/DF/df.AB_CVIDxHD.xlsx", asTable = TRUE, rowNames=TRUE)
# Theme
...
ggplot(data = df, aes(x = log2FoldChange, y = -log10(pvalue))) +
  ...
  labs(title="Activated Bright Memory B cells: CVID x HD")+
