...
dds2 <-DESeq(dds)
resultsNames(dds2)
res <- results(dds2, contrast = c("Group", "CVID_Activated_BrightMBCs", "CVID_Nonactivated_BrightMBCs"))

###pheatmap package###
#transform in data frame
DEGs=data.frame(res[(res$padj)<0.05,])
DEG2=DEGs%>%filter(abs(log2FoldChange) > 2.5)
write.xlsx(DEG2,"C:/Users/Anna/Documents/CVID_project/Venn/DEG2.xlsx", asTable = TRUE, rowNames=TRUE)
#save the data frame in excel
write.xlsx(DEGs,"C:/Users/Anna/Documents/CVID_project/DEG2s.xlsx", asTable = TRUE, rowNames=TRUE)
#Filter
top25_down <- DEGs %>%
...
s_mat2<-s_mat[, grep("^AB.CVID|^IB.CVID", names(s_mat))]
...
df_ann=ann_sample %>% 
  filter(str_detect(Sample, "^AB.CVID|^IB.CVID"))
df_ann <- data.frame(df_ann, row.names = 1)
df_ann$Group<-gsub("*_BrightMBCs", "", df_ann$Group)

##Colors for the groups
color_ann = list(df_ann, Group=c("CVID_Activated"  = "#048907",
                                 "CVID_Nonactivated" = "#890486"))
#heatmap
...
heat_plot <- pheatmap(s_mat2,scale="none", clustering_method="average", name="Z score",
                      ...
                      show_colnames = T,
                      fontsize_col = 6,
                      main = "CVID Bright Memory B cells: Activated x Nonactivated") 
##################################################################
###volcano plot###
df=data.frame(res)
write.xlsx(df,"C:/Users/Anna/Documents/CVID_project/DF/df.CVID_ABxIB.xlsx", asTable = TRUE, rowNames=TRUE)
# Theme
...
ggplot(data = df, aes(x = log2FoldChange, y = -log10(pvalue))) +
  ...
  labs(title="CVID Bright Memory B cells: Activated x Nonactivated")+
