...
dds2 <-DESeq(dds)
resultsNames(dds2)
res <- results(dds2, contrast = c("Group", "HD_Activated_BrightMBCs", "HD_Nonactivated_BrightMBCs"))
...
s_mat2<-s_mat[, grep("^AB.HD|^IB.HD", names(s_mat))]
...
df_ann=ann_sample %>% 
  filter(str_detect(Sample, "^AB.HD|^IB.HD"))
df_ann <- data.frame(df_ann, row.names = 1)
df_ann$Group<-gsub("*_BrightMBCs", "", df_ann$Group)

##Colors for the groups
color_ann = list(df_ann, Group=c("HD_Activated"  = "#048907",
                                 "HD_Nonactivated" = "#890486"))
#heatmap
...
heat_plot <- pheatmap(s_mat2,scale="none", clustering_method="average", name="Z score",
                      ...
                      show_colnames = T,
                      fontsize_col = 6,
                      main = "HD Bright Memory B cells: Activated x Nonactivated") 
##################################################################
###volcano plot###
df=data.frame(res)
write.xlsx(df,"C:/Users/Anna/Documents/CVID_project/DF/df.HD_ABxIB.xlsx", asTable = TRUE, rowNames=TRUE)
# Theme
...
ggplot(data = df, aes(x = log2FoldChange, y = -log10(pvalue))) +
  ...
  labs(title="HD Bright Memory B cells: Activated x Nonactivated")+
