...
countData_CVID <- countData[, grep("^IB.CVID|^IN.CVID|^AB.CVID|^AN.CVID", names(countData))]
####sample_table####
...
sampleTable$Group <- as.factor(paste(sampleTable$ID,sampleTable$CellType, sampleTable$Age, sampleTable$Sex,sampleTable$OnlyInfections,sampleTable$Status, sep = "_"))
###DESeq2###
dds <- DESeqDataSetFromMatrix(countData = countData_CVID,
                              colData = sampleTable,
                              design = ~ Group)
...
#ggplot
pcaData <- plotPCA(normal_dds, intgroup = c( "ID", "Sex", "OnlyInfections", "CellType", "Status"), returnData = TRUE)
attributes(pcaData)
...
pca2<-ggplot(pcaData, aes(x = PC1, y = PC2, color=interaction(OnlyInfections,Status), shape=CellType)) +
  geom_point(size =3) +
  geom_text(label=dds$ID,  nudge_x = 1, nudge_y = 1, color = "black", cex=2)+ 
  xlab(paste0("PC1: ", percentVar[1], "% variance")) +
  ylab(paste0("PC2: ", percentVar[2], "% variance")) +
  coord_fixed() +
  labs(shape="Cell Type", colour="Only Infections/Status")+
  scale_shape_manual(values = c(15,17,22,22,22,22))+
  ggtitle("PCA VST data - CVID") 
