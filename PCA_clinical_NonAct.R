...
countData_CVID_Nonactivated <- countData[, grep("^IB.CVID|^IN.CVID", names(countData))]
####sample_table####
...
sampleTable_CVID_Nonactivated = sampleTable %>% dplyr::filter(!grepl("^AB|^AN", Sample))
###DESeq2###
dds <- DESeqDataSetFromMatrix(countData = countData_CVID_Nonactivated,
                              colData = sampleTable_CVID_Nonactivated,
                              design = ~ Group)
...
pca2<-ggplot(pcaData, aes(x = PC1, y = PC2, color=interaction(OnlyInfections,CellType), shape=Sex)) +
  ...
  ggtitle("PCA Nonactivated Cells - Clinical Data") 
