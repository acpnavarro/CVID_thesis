...
dds2 <-DESeq(dds)
resultsNames(dds2)
res <- results(dds2, contrast = c("Group", "HD_Activated_NaïveBCs", "HD_Nonactivated_NaïveBCs"))
...
write.xlsx(DEGs,"C:/Users/Anna/Documents/CVID_project/DEGs.xlsx", asTable = TRUE, rowNames=TRUE)
#Filter
top25_down <- DEGs %>%
...
s_mat2<-s_mat[, grep("^AN.HD|^IN.HD", names(s_mat))]
...
df_ann=ann_sample %>% 
  filter(str_detect(Sample, "^AN.HD|^IN.HD"))
...
heat_plot <- pheatmap(s_mat2,scale="none", clustering_method="average", name="Z score",
                      ...
                      show_colnames = T,
                      fontsize_col = 6,
                      main = "HD Naïve B cells: Activated x Nonactivated") 
##################################################################
###volcano plot###
df=data.frame(res)
write.xlsx(df,"C:/Users/Anna/Documents/CVID_project/Venn/df.HD_ANxIN.xlsx", asTable = TRUE, rowNames=TRUE)
# Theme
...
ggplot(data = df, aes(x = log2FoldChange, y = -log10(pvalue))) +
  ...
  labs(title="HD Naïve B cells: Activated x Nonactivated")+
