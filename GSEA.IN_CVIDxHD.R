...
df <-read.xlsx("C:/Users/Anna/Documents/CVID_project/DF/df.IN_CVIDxHD.xlsx", rowNames=FALSE)
# log2 fold change 
...
write.xlsx(data_go,"C:/Users/Anna/Documents/CVID_project/GSEA/NonactNaive_CVIDxHD/GO.xlsx", asTable = TRUE, rowNames=TRUE)
...
p <- as.data.frame(gse) %>%
  ...
  ylab(NULL) +
  ggtitle("GO: Nonactivated Naïve CVID x HD")
...
write.xlsx(data_kegg,"C:/Users/Anna/Documents/CVID_project/GSEA/NonactNaive_CVIDxHD/KEGG.xlsx", asTable = TRUE, rowNames=TRUE)
...
#running score
plot<- gseaplot(kk2, geneSetID = 1, by = "runningScore", title = "KEGG: Nonactivated Naïve CVID x HD")
#pathview
library(pathview)
# Produce the native KEGG plot (PNG)
dme <- pathview(gene.data=entrez_geneList, pathway.id="hsa04659", species = kegg_organism)
...
write.xlsx(data_react,"C:/Users/Anna/Documents/CVID_project/GSEA/NonactNaive_CVIDxHD/Reactome.xlsx", asTable = TRUE, rowNames=FALSE)
...
p <- as.data.frame(react) %>%
  ...
  scale_fill_continuous(low='red', high='blue', guide=guide_colorbar(reverse=TRUE)) + 
  ylab(NULL) +
  ggtitle("Reactome: Nonactivated Naïve CVID x HD")
#upsetplot
upsetplot(react)
##DOSE
set.seed(123)
...
write.xlsx(data_do,"C:/Users/Anna/Documents/CVID_project/GSEA/NonactNaive_CVIDxHD/DO.xlsx", asTable = TRUE, rowNames=FALSE)
?gseDO
p <- as.data.frame(do) %>%
  mutate(
    num_genes = sapply(strsplit(core_enrichment, "/"), length),
...
write.xlsx(data_em2,"C:/Users/Anna/Documents/CVID_project/GSEA/NonactNaive_CVIDxHD/Motifs.xlsx", asTable = TRUE, rowNames=FALSE)
...
p <- as.data.frame(em2) %>%
  ...
  scale_fill_continuous(low='red', high='blue', guide=guide_colorbar(reverse=TRUE)) + 
  ylab(NULL) +
  ggtitle("Motifs: Nonactivated Naïve CVID x HD")
...
write.xlsx(data_em3,"C:/Users/Anna/Documents/CVID_project/GSEA/NonactNaive_CVIDxHD/Signatures.xlsx", asTable = TRUE, rowNames=FALSE)
...
p <- as.data.frame(em3) %>%
  ...
  scale_fill_continuous(low='red', high='blue', guide=guide_colorbar(reverse=TRUE)) + 
  ylab(NULL) +
  ggtitle("Signatures: Nonactivated Naïve CVID x HD")
...
write.xlsx(data_em4,"C:/Users/Anna/Documents/CVID_project/GSEA/NonactNaive_CVIDxHD/Hallmarks.xlsx", asTable = TRUE, rowNames=FALSE)
...
p <- as.data.frame(em4) %>%
  ...
  scale_fill_continuous(low='red', high='blue', guide=guide_colorbar(reverse=TRUE)) + 
  ylab(NULL) +
  ggtitle("Hallmark pathways: Nonactivated Naïve CVID x HD")
