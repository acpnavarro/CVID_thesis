...
####sample_table####
sampleTable=read.xlsx("C:/Users/Anna/Documents/CVID_project/Excel_tables/table2_dds.xlsx")
...
dds2 <-DESeq(dds)
resultsNames(dds2)
res <- results(dds2, contrast = c("Group", "CVID_Nonactivated_NaïveBCs", "HD_Nonactivated_NaïveBCs"))
...
write.xlsx(DEGs,"C:/Users/Anna/Documents/CVID_project/DEGs/IN_CVIDxHD/pvalue0.05.xlsx", asTable = TRUE, rowNames=TRUE)
#Filter
top25_down <- DEGs %>%
...
write.xlsx(top50,"C:/Users/Anna/Documents/CVID_project/DEGs/IN_CVIDxHD/top50.xlsx", asTable = TRUE, rowNames=TRUE)
##########################################################################
#Filter
top7_down <- DEGs %>%
  filter(log2FoldChange < -1.5) %>%
...
top7_up<- DEGs %>%
  ...
  arrange(desc(abs(log2FoldChange)))  %>%
  dplyr::slice(1:7)
#data frame with 14 most DEGs according to log2FC
top14 <- rbind (top7_up, top7_down)
write.xlsx(top14,"C:/Users/Anna/Documents/CVID_project/DEGs/IN_CVIDxHD/top14.xlsx", asTable = TRUE, rowNames=TRUE)
############################################################################
##prepare the count table
mat<-assay(normal_dds)[rownames(top50),]
#Z score
s_mat=t(scale(t(mat)))
# select only the nonactivated naive cell samples patients from the count table
s_mat=as.data.frame(s_mat)
s_mat2<-s_mat[, grep("^IN", names(s_mat))]
...
df_IN=df_clinical %>% 
  filter(str_detect(row.names(df_clinical), "^IN"))
df_ann2=cbind(df_ann,df_IN)
...
heat_plot <- pheatmap(s_mat2,scale="none", clustering_method="average", name="Z score",
                      ...
                      show_colnames = T,
                      fontsize_col = 6,
                      main = "Nonactivated Naïve B cells") 
draw(heat_plot, heatmap_legend_side="right")
##################################################################
###volcano plot###
df=data.frame(res)
write.xlsx(df,"C:/Users/Anna/Documents/CVID_project/DF/df.IN_CVIDxHD.xlsx", asTable = TRUE, rowNames=TRUE)
# Theme
...
ggplot(data = df, aes(x = log2FoldChange, y = -log10(pvalue))) +
  ...
  geom_point(aes(color=df$diffexpressed), size = 1.5, shape=20 ) +
  scale_color_manual(values = c("#00AFBB", "grey", "#e2062c"),
                     labels = c("Downregulated", "Not significant", "Upregulated"))+
  coord_cartesian(ylim = c(0, 160), xlim = c(-10, 10)) +
  ...
  labs(title="Nonactivated Naïve B cells: CVID x HD")+
